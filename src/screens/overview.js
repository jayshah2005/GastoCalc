...
const Overview = () => {
  ...
  const ComparisionText = () => <>{prevTotalExpenseOfMonth >= TotalExpenseOfMonth ? (
        prevTotalExpenseOfMonth == TotalExpenseOfMonth ? (
          <View><Text>You spent exactly the same as last month</Text></View>
        ) : (
  ...
  return (
    <View style={styles.container}>
      {TotalExpenseOfMonth === 0 ? (
        <View><Text style={styles.graphabsence}>Currently you have no expenses for this month</Text></View>
      ) : (
        <>
          <DonutGraph />
          <ComparisionText />
